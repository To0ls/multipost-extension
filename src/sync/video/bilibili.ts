...
export async function VideoBilibili(data: SyncData) {
  ...
  function findCoverEntry(): { element: HTMLElement; isAibeikeFallback: boolean } | null {
    const existingEntry = document.querySelector("div.cover-main-img > div.img") as HTMLElement | null;
    if (existingEntry) return { element: existingEntry, isAibeikeFallback: false };
    ...
    if (fallbackEntry) return { element: fallbackEntry, isAibeikeFallback: true };
  ...
  async function uploadCover(cover: FileData): Promise<boolean> {
    ...
    await sleep(coverEntry.isAibeikeFallback ? 1500 : 1000);
