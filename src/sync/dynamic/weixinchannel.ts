...
export async function DynamicWeiXinChannel(data: SyncData) {
  ...
  async function findDescriptionEditor(): Promise<HTMLDivElement | null> {
    const aibeikeSelector = 'div[data-placeholder="添加描述, 1000个字符内"]';
    const legacySelector = "div.input-editor";
    const aibeikeEditor = queryElement(aibeikeSelector) as HTMLDivElement | null;
    const legacyEditor = queryElement(legacySelector) as HTMLDivElement | null;
    if (aibeikeEditor || legacyEditor) return aibeikeEditor || legacyEditor;

    await waitForElementOptional(`${aibeikeSelector}, ${legacySelector}`, 1500);
    const waitedAibeikeEditor = queryElement(aibeikeSelector) as HTMLDivElement | null;
    const waitedLegacyEditor = queryElement(legacySelector) as HTMLDivElement | null;
    return waitedAibeikeEditor || waitedLegacyEditor;
  }
