 ...
 *
 * aibeike 1.6.5 uses signed COS upload plus the draft API. This implementation keeps the DOM fill path.
 */
...
export async function ArticleTencentyun(data: SyncData) {
  ...
  async function uploadCover(cover?: FileData): Promise<boolean> {
    ...
    if (!fileInput) {
      // TODO: aibeike only exposes cover upload through COS tmp-secret plus addArticleDraft API; no safe DOM selector was present in the reference bundle.
  ...
  try {
    // TODO: Do not port aibeike's COS tmp-secret upload and addArticleDraft API without verifying session portability.
