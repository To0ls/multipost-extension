 ...
 *
 * Based on the aibeike 1.6.5 DOM path. This still needs live validation on the target platform.
 */
export async function ArticleKaiDiWang(data: SyncData) {
  ...
  async function uploadInlineImage(fileData: FileData): Promise<string | null> {
    ...
    try {
      const response = await fetch("https://www.9kd.com/api/kd-group/oss/uploadFile", {
        method: "POST",
        body: formData,
        headers: {},
      });
  ...
  function closeGuideIfPresent(): void {
    const closeButton = document.querySelector("div.cx") as HTMLElement | null;
    if (closeButton) {
      closeButton.click();
    }
  }
  ...
  try {
    ...
    await waitForElementOptional("input");
    await sleep(1000);

    closeGuideIfPresent();
    await sleep(1000);

    const titleInput = document.querySelector('input[placeholder="请输入标题(5-60个字)"]') as HTMLInputElement | null;
    const title = articleData.title?.slice(0, 60) || "";
    if (titleInput && title) {
      try {
